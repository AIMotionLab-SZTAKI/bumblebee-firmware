...
typedef struct {
  ...
  uint8_t stateForLog;
  uint32_t conditionsBitfield;
  uint8_t emergencyStopSource;
  uint32_t setpointAge;
...
static supervisorConditionBits_t updateAndPopulateConditions(SupervisorMem_t* this, const sensorData_t *sensors, const setpoint_t* setpoint, const uint32_t currentTick) {
  ...
  this->emergencyStopSource = 0;
  if (!checkEmergencyStopWatchdog(currentTick)) {
    this->emergencyStopSource |= 0x01;
  }
  if (locSrvIsEmergencyStopRequested()) {
    this->emergencyStopSource |= 0x02;
  }
  if (this->paramEmergencyStop) {
    this->emergencyStopSource |= 0x04;
  }
  if (this->emergencyStopSource) {
...
static void updateLogData(SupervisorMem_t* this, const supervisorConditionBits_t conditions) {
  ...
  this->stateForLog = this->state;
  this->conditionsBitfield = conditions;
...
/**
 * @brief Current supervisor state enum value
 */
LOG_ADD(LOG_UINT8, state, &supervisorMem.stateForLog)
/**
 * @brief Current supervisor condition bitfield (SUPERVISOR_CB_*)
 */
LOG_ADD(LOG_UINT32, conds, &supervisorMem.conditionsBitfield)
/**
 * @brief Emergency stop source bitfield (1=watchdog, 2=CRTP request, 4=stabilizer.stop)
 */
LOG_ADD(LOG_UINT8, emStopSrc, &supervisorMem.emergencyStopSource)
/**
 * @brief Commander setpoint age in ticks
 */
LOG_ADD(LOG_UINT32, spAge, &supervisorMem.setpointAge)
